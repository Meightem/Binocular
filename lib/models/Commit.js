'use strict';

const _ = require('lodash');
const Promise = require('bluebird');
const log = require('debug')('git');
const aql = require('arangojs').aql;
const Model = require('./Model.js');
const File = require('./File.js');

const Commit = Model.define('Commit', {
  attributes: ['sha', 'message', 'signature', 'date', 'stats', 'webUrl'],
  keyAttribute: 'sha'
});

Commit.persist = function(nCommit, urlProvider = null) {
  const sha = nCommit.id().toString();

  return Commit.findById(sha).then(function(instance) {
    if (!instance) {
      log('Processing', sha);

      const parentShas = [];
      for (let i = 0; i < nCommit.parentcount(); i++) {
        parentShas.push(nCommit.parentId(i).toString());
      }

      return Commit.create({
        sha,
        signature: nCommit.committer().toString(),
        date: nCommit.date(),
        message: nCommit.message(),
        webUrl: urlProvider ? urlProvider.getCommitUrl(sha) : null,
        stats: {
          additions: 0,
          deletions: 0
        }
      })
        .tap(function(commit) {
          return Promise.join(
            commit.processTree(nCommit, urlProvider),
            Promise.map(parentShas, (parentSha,index) => {
              return Commit.findById(parentSha).then(parentCommit => commit.connect(parentCommit, {"orderIndex": index}));
            })
          );
        })
        .then(commit => [commit, true]);
    }

    log('Skipped', sha);

    return [instance, false];
  });
};

Commit.prototype.processTree = function(nCommit, urlProvider) {
  const self = this;

  return Promise.resolve(nCommit.getDiffWithOptions({ contextLines: 0 })).map(diff => {
    return Promise.resolve(diff.patches())
      .map(patch => {
        const newFile = patch.newFile().path();
        const stats = patch.lineStats();

        self.stats = {
          additions: stats.total_additions,
          deletions: stats.total_deletions
        };

        return Promise.join(
          File.ensureByPath(newFile, { webUrl: urlProvider.getFileUrl('master', newFile) }).spread(
            f => f
          ),
          Promise.resolve(nCommit.getEntry(newFile))
            .then(e => e.getBlob())
            .then(b => _.sumBy(b.toString(), ch => ch === '\n'))
            .catch(/the path '.*' does not exist in the given tree/, () => {
              return 0;
            }),
          Promise.resolve(patch.hunks()).map(hunk => ({
            webUrl: urlProvider
              ? urlProvider.getHunkUrl(nCommit.id(), newFile, hunk.newStart(), hunk.newLines())
              : null,
            newLines: hunk.newLines(),
            newStart: hunk.newStart(),
            oldLines: hunk.oldLines(),
            oldStart: hunk.oldStart()
          })),
          self.save()
        ).spread((file, lineCount, hunks) => ({ file, lineCount, hunks }));
      })
      .map(patch => {
        return self.connect(patch.file, {
          lineCount: patch.lineCount,
          hunks: patch.hunks
        });
      });
  });
};

Commit.prototype.childCommits = function() {
  const CommitCommitConnection = require('./CommitCommitConnection.js');
  const sha = this.data.sha;
  return Promise.resolve(
    Commit.rawDb.query(
      aql`FOR c IN ${Commit.collection}
            FILTER c.sha == ${sha}
            FOR child IN 1..1 OUTBOUND c ${CommitCommitConnection.collection} RETURN child
        `
    )
  ).then(cursor => cursor.all());
}

Commit.prototype.parentCommits = function() {
  const CommitCommitConnection = require('./CommitCommitConnection.js');
  const sha = this.data.sha;
  return Promise.resolve(
    Commit.rawDb.query(
      aql`FOR c IN ${Commit.collection}
            FILTER c.sha == ${sha}
            FOR v, e,p IN 1..1 INBOUND c ${CommitCommitConnection.collection}
              RETURN {index: p.edges[*].orderIndex, sha: v}
        `
    )
  ).then(cursor => cursor.all());
}

Commit.prototype.getAllLinearParents = function() {
  const CommitCommitConnection = require('./CommitCommitConnection.js');
  const sha = this.data.sha;
  return Promise.resolve(
    Commit.rawDb.query(
      aql`let specialCommits = (
            FOR c IN ${Commit.collection}
              let parentCommitCount = COUNT(FOR other IN 1..1 INBOUND c ${CommitCommitConnection.collection} RETURN other)
              let childrenCommitCount = COUNT(FOR other IN 1..1 OUTBOUND c ${CommitCommitConnection.collection} RETURN other)
              FILTER (childrenCommitCount != 1) OR (parentCommitCount != 1)
                RETURN c
          )
          FOR commit IN ${Commit.collection}
            FILTER commit.sha == ${sha}
            FOR c, e, p IN 1..1000 INBOUND commit ${CommitCommitConnection.collection}
              PRUNE CONTAINS_ARRAY(specialCommits, c) AND (NOT (c == commit))
              OPTIONS {bfs: true, uniqueEdges : 'path', uniqueVertices : 'path'}
              FILTER CONTAINS_ARRAY(specialCommits, c) AND (NOT (c == commit))
                RETURN { "parents": SLICE(p.vertices[*].sha, 1, LENGTH(p.vertices) - 2) }
        `
    )
  ).then(cursor => {
    return cursor.all().then(result => {
      const firstElem = result[0];
      if(firstElem && firstElem.parents){
        return firstElem.parents;
      }
      return [];
    });
  });
}

Commit.getAllMergeCommits = function(){
  const CommitCommitConnection = require('./CommitCommitConnection.js');
  return Promise.resolve(
    Commit.rawDb.query(
      aql`FOR c IN ${Commit.collection}
          let ps = (FOR parent, e, p IN 1..1 INBOUND c ${CommitCommitConnection.collection} RETURN {sha: parent.sha, index: p.edges[0].orderIndex})
          FILTER (COUNT(ps) > 1)
            RETURN {commit: c, parents: ps}
        `
    )
  ).then(cursor => cursor.all());
}

Commit.deduceStakeholders = function() {
  const CommitStakeholderConnection = require('./CommitStakeholderConnection.js');
  const Stakeholder = require('./Stakeholder.js');

  // walk through all commits
  return Promise.resolve(
    Commit.rawDb.query(
      aql`
    FOR commit IN ${Commit.collection}
        LET stakeholders = (FOR stakeholder
                    IN
                    INBOUND commit ${CommitStakeholderConnection.collection}
                        RETURN stakeholder)
        FILTER LENGTH(stakeholders) == 0
        COLLECT sig = commit.signature INTO commitsPerSignature = commit
        RETURN {
          "signature": sig,
          "commits": commitsPerSignature
        }`
    )
  )
    .then(cursor => cursor.all())
    .each(function(signature) {
      // try to get an already existing stakeholder with that signature
      return Stakeholder.ensureByGitSignature(signature.signature).spread(function(stakeholder) {
        // walk over all commits with that signature
        return Promise.map(signature.commits, function(rawCommit) {
          // assign the commit to the stakeholder
          return Commit.parse(rawCommit).connect(stakeholder);
        });
      });
    });
};

module.exports = Commit;
