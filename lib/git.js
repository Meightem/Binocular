...
Repository.prototype.getAllBranchReferences= function(){
  return this.repo.getReferences(Git.Reference.TYPE.OID).then(values => {
    const branches = values.filter(ref => !ref.isTag());
    const localBranches = branches.filter(branch => !branch.isRemote());
    return branches.filter((branch) => 
        !localBranches.find((localBranch) =>{
          return "origin/"+localBranch.shorthand() === branch.shorthand()
        })
      );
  });
}
